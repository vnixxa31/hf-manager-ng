...
def get_repo_file_entries(repo_id: str, token: str | None = None) -> list[dict[str, object]]:
...
def download_worker() -> None:
        while True:
                ...
                try:
                        ...
                        try:
                                metadata = fetch_file_tracking_metadata(repo_id=repo_id, file_path=file_path, token=token)
                                size_bytes = metadata["size_bytes"] if isinstance(metadata["size_bytes"], int) else size_bytes
                                etag = metadata["etag"] if isinstance(metadata["etag"], str) else None
                                commit_hash = metadata["commit_hash"] if isinstance(metadata["commit_hash"], str) else None
...
def list_repo_files(payload: RepoRequest) -> dict[str, object]:
        ...
        try:
                ...
                raise HTTPException(status_code=400, detail=f"Failed to read repo: {exc}") from exc
