...
export default function App() {
  ...
  const [lastScrolledFilePath, setLastScrolledFilePath] = useState<
    string | null
  >(null)
  ...
  useEffect(() => {
    const refresh = async () => {
      try {
        const [queueRes, downloadsRes] = await Promise.all([
          getQueue(),
          getDownloads(),
        ])
        if (queueRes.ok) {
  ...
  const handleToggleAll = useCallback(
    (checked: boolean) => {
      ...
      setFiles((prev) =>
        prev.map((f) =>
          visiblePaths.has(f.path) ? { ...f, selected: checked } : f,
        ),
      )
    },
    [filteredFiles],
  ...
  const handleSelectVisible = useCallback(() => {
    ...
    setFiles((prev) =>
      prev.map((f) =>
        visiblePaths.has(f.path) ? { ...f, selected: true } : f,
      ),
    )
  }, [filteredFiles])
  ...
  const handleToggleQuant = useCallback(
    (q: string) => {
      ...
      if (hasMatches) {
        setFiles((prev) =>
          prev.map((f) =>
            f.quantizations.includes(q) ? { ...f, selected: true } : f,
          ),
        )
      }
      ...
      const visibleMatch = filteredFiles.find((f) =>
        f.quantizations.includes(q),
      )
  ...
  const handleDownloadFilterToggle = useCallback(
    (mode: "downloaded" | "not_downloaded") => {
